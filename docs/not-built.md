...
Work that was scoped properly and then stopped before any code. Kept because
the design is the expensive part and it is done; picking one up should not
start from a blank page.

Two entries have since left this file, and both departures are worth as much as
what remains:

* **Attract mode shipped** — `ui/js/attract-screen.js` and the `attract_pool`
  command. Its design left with it.
* **The cartridge shelf was built and rejected.** Tried, not good enough, moved
  to `parked.md` on 2026-09-04 with its design intact. Read that before
...
### 3. Attract mode — shipped

`ui/js/attract-screen.js` and `commands::attract_pool`.
Struck from this list 2026-09-04.
