...
> "cart shelf was tried but not to my satisfatory, keep parking it"

**What happened:** built, looked at, rejected. Not abandoned for lack of time
and not blocked on anything — the result was not good enough. That is a
different verdict from the entry below, and a far more expensive one to
rediscover by repeating it.

The design that produced it follows, unchanged, at the end of this file. It is
kept because the reasoning about the artwork is still correct; it was the
...
## The cartridge shelf — the design as it stood

*Was `cartridge-shelf.md`.*

Games shown as the physical thing — a 3D cartridge with its real label, turned
towards you, with a sound and an insert animation when you pick one. Since
built, and rejected -- see the top of this file.
