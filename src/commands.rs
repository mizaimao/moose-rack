...
pub fn set_config_field(field: String, value: String) -> CmdResult<String> {
    ...
    if field == "autofire_hz" {
        ...
        crate::config::set_table_number("config.toml", table, key, n.clamp(1, 30))
            .map_err(err)?;
    ...
    if value.trim().is_empty() && !boolean {
        crate::config::clear_table_entry("config.toml", table, key).map_err(err)?;
    ...
    if boolean {
        ...
        crate::config::set_table_entry("config.toml", table, key, value.trim())
            .map_err(err)?;
    }
...
pub fn set_list_art(state: &AppState, value: String) -> CmdResult<String> {
    ...
    crate::config::set_table_entry("config.toml", "media", "list_art", &value)
        .map_err(err)?;
...
pub fn set_icon_set(state: &AppState, dir: String) -> CmdResult<String> {
    crate::config::set_table_entry("config.toml", "icons", "set", &dir).map_err(err)?;
...
pub fn remove_icon_set(state: &AppState, dir: String) -> CmdResult<String> {
    ...
    if *active == dir {
        active.clear();
        drop(active);
        crate::config::set_table_entry("config.toml", "icons", "set", "").map_err(err)?;
    }
...
pub fn set_motion_shader(state: &AppState, value: String) -> CmdResult<String> {
    crate::config::set_table_entry("config.toml", "shaders", "motion", &value)
        .map_err(err)?;
...
pub fn set_system_choice(
    ...
    crate::config::set_table_entry("config.toml", table, &slug, &value).map_err(err)?;
...
pub fn set_game_core(state: &AppState, id: i64, core: String) -> CmdResult<String> {
    ...
    if core.is_empty() {
        crate::config::clear_table_entry("config.toml", "cores.per_game", &key)
            .map_err(err)?;
    } else {
        crate::config::set_table_entry("config.toml", "cores.per_game", &key, &core)
            .map_err(err)?;
    }
...
pub fn set_retroarch_root(path: String) -> CmdResult<String> {
    ...
    if path.is_empty() {
        ...
        crate::config::clear_table_entry("config.toml", "retroarch", "root").map_err(err)?;
    ...
    crate::config::set_table_entry("config.toml", "retroarch", "root", &path)
        .map_err(err)?;
...
pub fn set_icon_style(state: &AppState, key: String) -> CmdResult<String> {
    ...
    *state.icon_look.lock().map_err(err)? = key.clone();
    ...
    crate::config::set_table_entry("config.toml", "icons", "style", &key).map_err(err)?;
    Ok(label)
}
...
pub fn set_picker_order(state: &AppState, kind: String, order: String) -> CmdResult<()> {
    ...
    crate::config::set_table_entry("config.toml", "picker_order", &kind, &order)
        .map_err(err)
}
