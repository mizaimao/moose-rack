...
pub struct CoreMap {
    ...
    pub default_core_by_server_platform: BTreeMap<String, String>,
...
pub struct System {
    ...
    pub server_platforms: Vec<String>,
...
/// Every system ES-DE names, upstream, generated by
/// `tools/extract_esde_names.py` from the bundled `es_systems.xml`.
///
/// 6 KB against the core map's 110, and it settles a question the core map was
/// never the right place for. That file is an *extract* -- 38 systems whose
/// emulator preferences were worth recording -- so a console outside it scanned
/// perfectly and then drew its own folder name on the card. `saturn` did, and
/// `wiiu`, `xbox360`, `naomi`, `pico8`, `easyrpg` and `gameandwatch` were all
/// one directory away from doing the same.
const ESDE_NAMES: &str =
    include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/data/esde-system-names.json"));

fn esde_names() -> &'static std::collections::BTreeMap<String, String> {
    static NAMES: std::sync::OnceLock<std::collections::BTreeMap<String, String>> =
        std::sync::OnceLock::new();
    NAMES.get_or_init(|| serde_json::from_str(ESDE_NAMES).unwrap_or_default())
}

/// What ES-DE calls a system, by its own directory name.
///
/// Tried with the hyphen removed too: our directory for PICO-8 is `pico-8` and
/// ES-DE's own name for it is `pico8`.
pub fn esde_name(system: &str) -> Option<&'static str> {
    let names = esde_names();
    names
        .get(system)
        .or_else(|| names.get(&system.replace('-', "")))
        .map(String::as_str)
}
...
impl CoreMap {
    ...
    pub fn display_name(&self, slug: &str) -> Option<&str> {
        ...
        if let Some(name) = self
            .systems
            .values()
            .find(|sys| sys.server_platforms.iter().any(|p| p == slug))
            .and_then(|sys| sys.fullname.as_deref())
        {
            return Some(name);
        }
        // Then ES-DE's own list, which names all 195 rather than our 38.
        esde_name(slug)
    }

    /// What to show on the card, given both names a scan knows.
    ///
    /// A game carries the ES-DE directory it came from *and* the slug the rest
    /// of the app keys on, and the two name tables are keyed differently: the
    /// core map by slug, ES-DE's list by its own directory name. Game and Watch
    /// is the case that needs both -- the directory is `gameandwatch` and the
    /// slug is `g-and-w`, and only ES-DE's file has a name for it.
    ///
    /// One function so the lookup a card does is the lookup a test can check.
    pub fn name_for(&self, system: &str, slug: &str) -> Option<&str> {
        self.display_name(slug).or_else(|| esde_name(system))
    }
    ...
    pub fn default_core(&self, platform: &str) -> Option<&str> {
        self.default_core_by_server_platform
            .get(platform)
            .map(String::as_str)
    }
    ...
    pub fn alternatives(&self, platform: &str) -> Vec<&str> {
        ...
        for system in self.systems.values() {
            if !system.server_platforms.iter().any(|p| p == platform) {
                continue;
            }
    ...
    pub fn platforms_with_core(&self, core: &str) -> Vec<String> {
        ...
        for system in self.systems.values() {
            ...
            if has {
                for p in &system.server_platforms {
                    if !out.contains(p) {
                        out.push(p.clone());
                    }
                }
...
mod tests {
    ...
    fn map() -> CoreMap {
        serde_json::from_str(
            r#"{
              "default_core_by_server_platform": {"snes": "snes9x"},
              "systems": {"snes": {
                "server_platforms": ["snes"],
                "emulators": [
    ...
    /// invisible until a library held one of the other 157: `saturn` scanned
    /// perfectly, through `SYSTEM_ALIASES`, and rendered as lowercase "saturn"
    /// beside twenty consoles with real names.
    #[test]
    fn every_aliased_system_has_a_name() {
        let map = CoreMap::embedded();
        // The directory names `esde::SYSTEM_ALIASES` accepts, and the slugs
        // they resolve to -- both have to name something.
        for (system, slug) in [
            ("genesis", "megadrive"),
            ...
            // Through `name_for`, which is what the scan calls. Asserting the
            // two halves separately passed with the fallback ripped out.
            assert!(
                map.name_for(system, slug).is_some(),
                "{system} scans but has no display name, so its card draws the folder name"
            );
        }
    }

    /// The names are ES-DE's own, not ours.
    #[test]
    fn the_names_come_from_esde() {
        assert_eq!(esde_name("saturn"), Some("Sega Saturn"));
        assert_eq!(esde_name("wiiu"), Some("Nintendo Wii U"));
        assert_eq!(esde_name("xbox360"), Some("Microsoft Xbox 360"));
        // Our directory is `pico-8`; ES-DE spells it `pico8`.
        assert_eq!(esde_name("pico-8"), Some("PICO-8 Fantasy Console"));
        assert!(esde_names().len() > 150, "only {} names loaded", esde_names().len());
        // Not a system anywhere, and not invented.
        assert_eq!(esde_name("nes_unlicensed"), None);
    }

    /// The core map still wins where it has an opinion: it says
    /// "Nintendo SNES (Super Nintendo)" where ES-DE says something else, and
    /// that is the name this library has always shown.
    #[test]
    fn the_core_map_is_still_the_first_word() {
        let map = CoreMap::embedded();
...
impl CoreMap {
    ...
    pub fn knows_platform(&self, platform: &str) -> bool {
        self.systems
            .values()
            .any(|s| s.server_platforms.iter().any(|p| p == platform))
    }
    ...
    pub fn android_launches(&self, platform: &str) -> Vec<AndroidLaunch> {
        let Some(sys) = self
            .systems
            .values()
            .find(|s| s.server_platforms.iter().any(|p| p == platform))
        else {
            return Vec::new();
        };
...
mod android_tests {
    ...
    fn a_standalone_emulator_becomes_its_own_component() {
        ...
        let found = m
            .systems
            .values()
            .flat_map(|s| s.server_platforms.clone())
